...
/* Fase 5 · TANDA 4 — pesos de Playfair acotados a los realmente usados:
   400 (regular) + italic 400 (cita Hero, blockquote AboutUs, wordmark). Antes
   se cargaban 7 pesos (400-900 + italic) y solo se usaba el 400 + italic.
   El recorte ahorra ~3 archivos de fuente (subset latin), peso neto al
   primer paint. */
...
/* Fase 5 · TANDA 4 — dominio canónico `yayamariana.es` (no comprado todavía).
   El deploy `yaya-mariana.vercel.app` es solo para revisión visual familiar
   — no quiere indexarse como canonical. Cuando se compre el dominio real
   este string se mantiene; basta apuntar el DNS. */
...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    description: SITE_DESCRIPTION,
    /* La OG image la sirve la convención `opengraph-image.tsx` — Next la
       inyecta automáticamente con dimensiones y alt; no hace falta listarla
       aquí (si se duplica, Next sirve dos og:image, lo cual confunde a los
  ...
  /* No `authors`/`creator`/`publisher`: el fundador de registro es José Elías
     Navarro y NO aparece en superficies públicas (HARD RULE de project
     memory). La marca "Yaya Mariana" es la única autoridad visible. */
};

/* JSON-LD — Organization + 3 Products. Inyectado en `<head>` por
   `<script type="application/ld+json">`. La cláusula `founder` queda
   deliberadamente fuera del schema por la HARD RULE; el resto de los
   campos refleja los datos públicos ya visibles en la home. */
...
const jsonLdProducts = PRODUCTS.map((p) => ({
  ...
  /* `additionalProperty` para la variedad sin ensuciar el `name`: útil para
     que crawlers como Google Shopping resuelvan los 3 SKUs como variantes. */
  additionalProperty: [
    ...
    >
      <head>
        ...
        <script
          ...
          /* `dangerouslySetInnerHTML` es el patrón estándar Next.js para
